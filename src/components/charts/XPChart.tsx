...
export function XPChart({ data }: { data: any[] }) {
  ...
  const points = cumulativeXP.map((d) => {
    const x = margin.left + (width * (d.date.getTime() - xMin.getTime()) / (xMax.getTime() - xMin.getTime()));
  ...
  const xTicks = [0, 0.25, 0.5, 0.75, 1].map(percent => {
    const date = new Date(xMin.getTime() + percent * (xMax.getTime() - xMin.getTime()));
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        <polygon
          fill="url(#xpGradient)"
          points={`${margin.left},${height + margin.top} ${points} ${width + margin.left},${height + margin.top}`}
        />
      </svg>
    </div>
  );
