...
export function XPByProjectChart({ data }: { data: any[] }) {
  ...
  useEffect(() => {
    ...
    data.forEach((item) => {
      ...
      const projectName = pathParts.length > 2 ? pathParts[pathParts.length - 2] : 'Unknown';
    ...
    <div className="w-full">
      ...
      >
        ...
        {projectsXP.map((project, i) => {
          const x = margin.left + (width / projectsXP.length) * i + (width / projectsXP.length - barWidth) / 2;
          ...
          return (
            <g key={i}>
              ...
              <text
                ...
                fontSize="10"
                fill="currentColor"
                transform={`rotate(45, ${x + barWidth / 2}, ${height + margin.top + 15})`}
              >
                {project.name}
              </text>
            </g>
