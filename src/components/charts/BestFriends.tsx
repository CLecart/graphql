...
const BestFriendsComponent: React.FC<Props> = ({ data, currentUserLogin }) => {
  ...
  const { bestFriends, newConnections, totalUniqueConnections } = processFriendsData(
    data.user[0].groups,
    currentUserLogin
  );

  // Calculate progress (every 10 people)
  const progressPercentage = Math.min(100, Math.floor((totalUniqueConnections % 10) * 10));
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg">
      ...
      <div className="mb-8 p-4 bg-purple-50 rounded-lg">
        <div className="flex justify-between items-center mb-2">
          <h2 className="text-lg font-semibold text-purple-800">People you've worked with</h2>
          <span className="text-purple-600 font-bold">{totalUniqueConnections}</span>
        </div>
        ...
        <p className="text-sm text-purple-500 mt-2">
          {progressPercentage > 0 ? (
            <>Only {10 - (totalUniqueConnections % 10)} more to reach {milestone} people!</>
          ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="bg-blue-50 p-6 rounded-lg">
          <h2 className="text-xl font-semibold mb-4 text-blue-800">Top 5 Best Friends</h2>
              ...
              {bestFriends.map((friend, index) => (
                <li key={friend.login} className="flex items-center">
                  ...
                  <span className="text-gray-700">
                    {friend.login} <span className="text-blue-600">({friend.count} groups together)</span>
                  </span>
                </li>
              ))}
            </ul>
        ...
        <div className="bg-green-50 p-6 rounded-lg">
          <h2 className="text-xl font-semibold mb-4 text-green-800">Latest New Connections</h2>
            ...
            <ul className="space-y-3">
              {newConnections.map((person, index) => (
                <li key={person.login} className="flex items-center">
                  ...
                  <span className="text-gray-700">
                    {person.login} <span className="text-green-600">(since {new Date(person.firstMet).toLocaleDateString()})</span>
                  </span>
                </li>
              ))}
            </ul>
...
function processFriendsData(groups: GroupData[], currentUserLogin: string) {
  ...
  const sortedNewConnections = uniqueConnections
    .sort((a, b) => new Date(b.firstMet).getTime() - new Date(a.firstMet).getTime())
    .slice(0, 5);
